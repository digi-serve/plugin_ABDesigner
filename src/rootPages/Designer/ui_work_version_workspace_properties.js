...
export default function (AB) {
   const ibase = "ui_work_version_workspace_properties";
   ...
   class UI_Work_Datacollection_Workspace_Properties extends UIClass {
      constructor() {
         ...
         this._templateListItem = `
            <div class='ab-page-list-item'>
               {common.icon()} <span class='webix_icon fa fa-#typeIcon#'></span> #label# #hasVersion#
            </div>
         `;
      ...
      applicationLoad(application) {
         ...
         const addPage = (page, index, parentId) => {
            ...
            const branch = {
               ...
               version: {
                  id: page.version ? page.version.id : "",
               },
            };
      ...
      initLinkDatacollectionOptions() {
         ...
         if (objSource) {
            ...
            const linkDCs = this.CurrentApplication.versionsIncluded().filter(
               (dc) => linkObjectIds.includes(dc?.settings.datasourceID)
            );
      ...
      initLinkFieldOptions(linkedDvId = null) {
         ...
         const linkDC = linkedDvId ? this.AB.versionByID(linkedDvId) : null;
      ...
      populatePopupEditors() {
         const version = this.CurrentDatacollection || null;
         const datasource = version?.datasource || null;

         if (datasource) {
            // array of filters to apply to the data table
            const filterConditions = version?.settings?.objectWorkspace
               ?.filterConditions || { glue: "and", rules: [] };

            const sortConditions =
               version?.settings?.objectWorkspace?.sortFields || [];
      ...
      populateBadgeNumber() {
         const ids = this.ids;
         const version = this.CurrentDatacollection;

         const $buttonFilter = $$(ids.buttonFilter);
         const $buttonSort = $$(ids.buttonSort);

         if (version?.settings?.objectWorkspace?.filterConditions?.rules) {
            $buttonFilter.define(
               "badge",
               version.settings.objectWorkspace.filterConditions.rules
                  ?.length || null
            );
         ...
         if (version?.settings?.objectWorkspace?.sortFields) {
            $buttonSort.define(
               "badge",
               version.settings.objectWorkspace.sortFields.length || null
            );
            $buttonSort.refresh();
         } else {
      ...
      onSortChange(sortSettings) {
         ...
         version.settings.objectWorkspace =
            version.settings.objectWorkspace || {};
         // store sort settings
         version.settings.objectWorkspace.sortFields = sortSettings || [];
      ...
      templateListItem(item, common) {
         ...
         if (
            ...
            item.version.id == this.CurrentDatacollection.id
         ) {
            hasVersion =
               "<i class='webix_icon hasVersion fa fa-check-circle'></i>";
         }
